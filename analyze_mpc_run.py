...
def analyze_latest_run():
    ...
    print("\n🎮 CONTROL COMMANDS")
    print(
        f"  Roll (φ_c):   min={min(log['phi_c']):+.4f}, max={max(log['phi_c']):+.4f}, "
        f"mean={np.mean(log['phi_c']):+.4f}"
    )
    print(
        f"  Pitch (θ_c):  min={min(log['theta_c']):+.4f}, max={max(log['theta_c']):+.4f}, "
        f"mean={np.mean(log['theta_c']):+.4f}"
    )
    print(
        f"  Yaw (ψ_c):    min={min(log['psi_c']):+.4f}, max={max(log['psi_c']):+.4f}, "
        f"mean={np.mean(log['psi_c']):+.4f}"
    )
    print(
        f"  Thrust (T_c): min={min(log['thrust_c']):.4f}, max={max(log['thrust_c']):.4f}, "
    ...
    # Speed statistics
    print("\n🚀 SPEED TRACKING")
    print("  Target speed: ~4.0 m/s")
    print(
        f"  Achieved ds: min={min(log['ds']):.3f}, max={max(log['ds']):.3f}, "
        f"mean={np.mean(log['ds']):.3f}"
    )

    print(f"\n{'=' * 70}")
    print(f"📁 Output directory: {parent_dir}")
    print("  - control_values.png (6-subplot figure)")
    print("  - solver_status.png (success/failure timeline)")
    print("  - control_log.json (raw data)")
    print(f"{'=' * 70}\n")
