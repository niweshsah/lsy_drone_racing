    ...
    def load_params(model_type, config_node):
        ...
        models = {
            "cf21B_500": {
                ...
                "arm_length": 0.035355,  # 'L' in config
                "cm": 0.00593893393599368,  # 'thrust2torque'
...
def export_spatial_drone_model(drone_params: dict):
    ...
    arm_length = drone_params.get("arm_length", 0.25)
    # 'thrust2torque' in user config is 'cm'
    Cm = drone_params.get("cm", 0.01)
...
class SpatialMPC:
    def __init__(
        self, N=20, Tf=1.0, max_thrust=None, tunnel_radius=0.4, safety_margin=0.05, v_target=5.0
    ):
        self.N = N
        self.Tf = Tf
    ...
    def solve(self, x0, geometry_engine):
        ...
        for k in range(self.N):
            ...
            p_val = np.concatenate(
                [[frame["k1"], frame["k2"]], frame["t"], frame["n1"], frame["n2"]]
            )
        ...
        p_val_e = np.concatenate(
            [[frame_e["k1"], frame_e["k2"]], frame_e["t"], frame_e["n1"], frame_e["n2"]]
        )
...
if __name__ == "__main__":
    ...
    # Initialize MPC
    mpc = SpatialMPC(N=20, Tf=1.0, tunnel_radius=0.5, safety_margin=0.05, v_target=20.0)
    ...
    for i in range(sim_steps):
        ...
        if i % 10 == 0:
            print(
                f"{i:<5} | {x_curr[0]:<8.2f} | {x_curr[3]:<8.2f} | {x_curr[1]:<6.2f} | {x_curr[2]:<6.2f} | {status}"
            )
