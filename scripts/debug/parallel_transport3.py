...
class GeometryEngine:
    ...
    def _determine_detour_direction(self, v_proj, y_axis, z_axis):
        ...
        if -45 <= proj_angle_deg < 45:
            return y_axis
        elif 45 <= proj_angle_deg < 135:
            return z_axis
        elif -135 <= proj_angle_deg < -45:
            return -z_axis
        else:
            return -y_axis
    ...
    def _generate_augmented_path(self):
        ...
        if np.linalg.norm(t_start) > 1e-6:
        ...
        for i in range(num_gates):
            ...
            tube_len_in = np.clip(
                dist_in * self.tube_length_ratio, self.tube_min_len, self.tube_max_len
            )
            ...
            tube_len_out = np.clip(
                dist_out * self.tube_length_ratio, self.tube_min_len, self.tube_max_len
            )
            ...
            if i < num_gates - 1:
                ...
                tube_len_in_next = np.clip(
                    dist_connector * self.tube_length_ratio, self.tube_min_len, self.tube_max_len
                )
    ...
    def plot(self):
        ...
        ax.scatter(
            self.aug_waypoints[:, 0],
            self.aug_waypoints[:, 1],
            self.aug_waypoints[:, 2],
            c="k",
            s=10,
        )
        ...
        for info in self.detour_info:
            ...
            # Draw Detour Point
            ax.scatter(
                pt_detour[0], pt_detour[1], pt_detour[2], c="orange", marker="X", s=150, zorder=10
            )
            ax.text(
                pt_detour[0],
                pt_detour[1],
        ...
        ax.set_title("Visualizing Path vs Normal Deviation")
        plt.show()
