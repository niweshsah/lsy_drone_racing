...
class GeometryEngine:
    ...
    def _compute_smoothed_tangents(self, blend_strength=0.4):
        """
        Computes tangents that blend the 'natural' path flow with strict gate normals.
        Includes magnitude scaling to prevent loops (overshoot) in tight turns.
        """
        ...
        dists = np.linalg.norm(np.diff(self.waypoints, axis=0), axis=1)
        # Pad dists to handle start/end easily
        dists = np.concatenate(([dists[0]], dists, [dists[-1]]))

        # --- A. Start Tangent ---
        start_dir = self.waypoints[1] - self.waypoints[0]
        if np.linalg.norm(start_dir) > 1e-6:
            tangents[0] = start_dir / np.linalg.norm(start_dir)
        else:
            tangents[0] = np.array([1, 0, 0])
        ...
        for i in range(1, n_points):
            ...
            # 1. Basic Vectors
            v_in = curr_p - prev_p
            dist_in = np.linalg.norm(v_in)
            if dist_in > 1e-6: v_in /= dist_in
            
            if i < n_points - 1:
                next_p = self.waypoints[i+1]
                v_out = next_p - curr_p
                dist_out = np.linalg.norm(v_out)
                if dist_out > 1e-6: v_out /= dist_out
            else:
                v_out = v_in
                dist_out = dist_in

            # 2. Compute "Natural" Flow (Bisector)
            ...
            # 3. Get Strict Gate Normal
            # Note: Gate 0 is at index 1 in waypoints
            gate_idx = i - 1
            t_strict = self.gate_normals[gate_idx].copy()

            # 4. AUTO-FLIP: Ensure strict normal points somewhat in direction of travel
            # Check against natural flow
            if np.dot(t_strict, t_natural) < 0:
                t_strict = -t_strict
                # Update the stored normal so future laps/steps use consistent direction
                self.gate_normals[gate_idx] = t_strict

            # 5. Blend
            # If the strict normal is CRAZY different (>90 deg) from natural flow, 
            # ignore it to prevent loops.
            if np.dot(t_strict, t_natural) < 0.1:
                effective_blend = 0.0 # Trust geometry only
            else:
                effective_blend = blend_strength

            t_final = (effective_blend * t_strict) + ((1 - effective_blend) * t_natural)
            if np.linalg.norm(t_final) > 1e-6:
                t_final /= np.linalg.norm(t_final)
            
            # --- CRITICAL FIX FOR CURVATURE ---
            # Hermite splines will loop if tangent magnitude is > 3 * distance.
            # We scale the tangent magnitude by the minimum neighbor distance.
            # This "softens" the curve at the waypoint if segments are short.
            min_neighbor_dist = min(dist_in, dist_out)
            
            # Heuristic: If gates are closer than 1.5m, scale tangent down.
            # Otherwise, keep unit magnitude for consistent speed parametrization.
            if min_neighbor_dist < 1.5:
                # Scale factor ensures we don't request a turn sharper than distance allows
                scale = 1.0 * (min_neighbor_dist / 1.5)
                # Clamp scale to be reasonable
                scale = max(0.5, scale) 
                t_final *= scale
