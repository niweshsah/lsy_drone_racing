...
if __name__ == "__main__":
    ...
    print(
        f"{'Step':<5} | {'s [m]':<7} | {'v [m/s]':<7} | {'Lat Err':<7} | {'Thrust':<6} | {'Status'}"
    )
    ...
    ax3d.plot(
        ...
        linewidth=2,
        alpha=0.9,
        label="Followed MPC Path",
    )

    # Add Scatter points colored by Velocity to show speed profile
    sc = ax3d.scatter(
        p_flown[:, 0], p_flown[:, 1], p_flown[:, 2], c=vel_hist, cmap="plasma", s=20, alpha=0.8
    )
    ...
    # Final Plot Settings
    ax3d.set_title(
        f"Trajectory Analysis: Reference vs. Actual\nRMSE: {rmse:.3f}m | Max Dev: {max_dev:.3f}m"
    )
    ax3d.set_xlabel("X [m]")
    ax3d.set_ylabel("Y [m]")
    ax3d.set_zlabel("Z [m]")
