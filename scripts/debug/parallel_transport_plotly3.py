...
class GeometryEngine:
    def __init__(
        ...
        if self.total_length < 1e-3:
            self.total_length = 0.1
            ...
            if curr_type == 1:
                ...
                if dist > 1e-6:
                    ...
                    if angle_deg > self.DETOUR_ANGLE_THRESHOLD:
                        ...
                        detour_dir = (
                            self.gate_z[gate_idx]
                            if np.linalg.norm(proj) < 1e-3
                            else proj / np.linalg.norm(proj)
                        )
    ...
    def _apply_obstacle_avoidance(self, wps, types, normals):
        ...
        if np.any(dists < 1e-4):
            wps += np.random.normal(0, 1e-5, wps.shape)
        ...
        final_wps = list(wps)
        final_types = list(types)
        final_normals = list(normals)
        points_added = 0
    ...
    def _compute_hermite_tangents(self):
        ...
        for i in range(num_pts):
            ...
            base_scale = max(
                1e-3,
                min(
                    dist_prev if dist_prev > 0 else dist_next,
    ...
    def _generate_parallel_transport_frame(self, num_points=1000):
        ...
        n2_0 = (
            np.cross(t0, np.array([1, 0, 0]))
            if np.linalg.norm(np.cross(t0, g_vec)) < 1e-3
            else np.cross(t0, np.cross(g_vec, t0))
        )
