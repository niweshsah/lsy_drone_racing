...
# ----------------------------------------------------------------------------
# ## 🚀 RRT* Planner Implementation (Unchanged)
# ----------------------------------------------------------------------------
...
class RRTStarPlanner:
    ...
    class Node:
        ...
        def __init__(self, pos: NDArray[np.floating], parent: 'RRTStarPlanner.Node' | None = None, cost: float = 0.0):
            self.pos = pos
            self.parent = parent
    ...
    def plan(self, start: NDArray[np.floating], goal: NDArray[np.floating]) -> List[NDArray[np.floating]]:
        ...
        for i in range(self.max_iter):
            ...
            if new_node:
                ...
                if goal_node is None or new_node.cost < goal_node.cost - self._get_line_cost(new_node.pos, goal):
                    if self._get_line_cost(new_node.pos, goal) < self.step_size and \
                       not self._is_collision(new_node.pos, goal):
        ...
        if goal_node is None:
            ...
            if self._is_collision(closest_node.pos, goal):
                ...
                goal_node = self.Node(goal, parent=closest_node, cost=closest_node.cost + self._get_line_cost(closest_node.pos, goal))
    ...
    def _steer(self, from_pos: NDArray[np.floating], to_pos: NDArray[np.floating]) -> NDArray[np.floating]:
    ...
    def _choose_parent_and_rewire(self, nearest_node: Node, new_point: NDArray[np.floating]) -> Node | None:
        """
        RRT* Core:
        ...
        for near_node in near_nodes:
            cost_via_near = self._get_cost(near_node) + self._get_line_cost(near_node.pos, new_point)
        ...
        for near_node in near_nodes:
            ...
            cost_via_new = self._get_cost(new_node) + self._get_line_cost(new_node.pos, near_node.pos)
            
            if cost_via_new < self._get_cost(near_node) and not self._is_collision(new_node.pos, near_node.pos):
...
# ----------------------------------------------------------------------------
# ## UPDATE: Modified Controller with Path Caching
# ----------------------------------------------------------------------------
...
class PathFollowingController(Controller):
    ...
    MODIFIED: This controller now saves the generated path to a .npy file
    and loads it on subsequent runs to avoid re-planning.
    """

    # Define the filename for the cached path.
    # This file will be saved in the directory where the script is executed.
    _PATH_FILENAME = "rrt_star_path.npy"
    ...
    def __init__(self, obs: dict[str, NDArray[np.floating]], info: dict, config: ConfigDict):
        ...
        # --- MODIFIED: Path Generation (Cache Check) ---
        
        # Check if the pre-computed path file exists
        if os.path.exists(self._PATH_FILENAME):
            print(f"Loading pre-computed path from {self._PATH_FILENAME}...")
            self._waypoints = np.load(self._PATH_FILENAME)
            print(f"Path loaded with {len(self._waypoints)} waypoints.")

        else:
            # --- Path Generation (RRT*) ---
            print(f"No path file found at {self._PATH_FILENAME}. Running RRT* planner...")
            
            # Get gate and obstacle info from config
            gate_centers = [np.array(g['pos']) for g in config.env.track.gates]
            obstacle_positions = [np.array(o['pos']) for o in getattr(config.env.track, "obstacles", [])]
            ...
            for i in range(len(key_waypoints) - 1):
                ...
                print(f"RRT*: Planning segment {i+1}/{len(key_waypoints)-1} (from {np.round(start,1)} to {np.round(goal,1)})...")
            ...
            # --- NEW: Save the path ---
            try:
                print(f"Saving generated path to {self._PATH_FILENAME}...")
                np.save(self._PATH_FILENAME, self._waypoints)
                print("Path saved successfully.")
            except Exception as e:
                print(f"Error saving path file: {e}")
        
        # --- Phase 2: Follow Path ---
        # This logic is now common, whether path was loaded or generated
        self._segment_ticks = 10  # Ticks per RRT* segment (10 ticks @ 50Hz = 0.2s)
