...
class Level0Controller(Controller):
    ...
    def compute_control(self, obs: dict[str, NDArray[np.floating]], info: dict | None = None) -> NDArray[np.floating]:
        ...
        vel = np.array([self._spline_x.derivative()(t),
                        self._spline_y.derivative()(t),
                        self._spline_z.derivative()(t)], dtype=np.float32)
        acc = np.array([self._spline_x.derivative(2)(t),
                        self._spline_y.derivative(2)(t),
                        self._spline_z.derivative(2)(t)], dtype=np.float32)
