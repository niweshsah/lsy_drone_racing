...
class GeometryEngine:
    def __init__(
        self, gates_pos: List[List[float]], gates_normals: List[List[float]], start_pos: List[float]
    ):
...
class SpatialMPCController(Controller):
    def __init__(self, obs: dict, info: dict, config: dict, env=None):
        ...
        # print("keys in config:", config['env'].keys())
        # print("gates list from config:", gates_list)

        self.OBS_radius = safety_radius
        self.W1_max = max_lateral_width
        self.W2_max = max_lateral_width

        obstacles_list = config.get("env", {}).get("track", {}).get("obstacles", [])
        # print("obstacles list from config:", obstacles_list)

        self.obstacles_pos = [obs["pos"] for obs in obstacles_list]
        # print("obstacles positions:", self.obstacles_pos)
    ...
    def _compute_corridor_bounds(self, s_pred, frame_pos, frame_t, frame_n1, frame_n2):  # noqa: ANN202
    ...
    def compute_control(self, obs: dict, info: dict | None = None) -> np.ndarray:
        ...
        for k in range(self.mpc.N):
            ...
            # Concatenate spatial bounds with angle bounds
            # Vector size must be 5: [w1, w2, phi, theta, psi]
            lbx_k = np.concatenate([w_lb, ANGLE_LB])
            ubx_k = np.concatenate([w_ub, ANGLE_UB])

            # Update Solver Constraints [cite: 590]
            # "Constraints... are enforced using the flight corridor... as s-dependent state bounds"
        ...
        print(
            f"Terminal Step, s_end: {s_end:.2f}, k_mag: {k_mag:.3f}, target vel: {target_vel:.3f} v_corner: {v_corner:.3f}, v_ref_k: {v_ref_k:.3f}"
        )
        ...
        if self.env is not None:
            try:
                ...
                draw_line(
                    ...
                    min_size=4.0,
                    max_size=4.0,  # Slightly thicker than ref path
                )
            except Exception as e:
                if self.debug:
    ...
    def plot_control_values(self, figsize=(16, 10), save_path: str = None):
        ...
        ax.axhline(
            y=self.params["mass"] * self.params["g"],
            color="k",
            linestyle="--",
            label="Hover Thrust",
            alpha=0.5,
    ...
    def plot_solver_status(self, save_path: str = None):
        ...
        ax.set_title(
            "MPC Solver Status Over Time (Green=Success, Red=Failure)",
            fontsize=12,
            fontweight="bold",
        )
        ax.grid(True, alpha=0.3)
    ...
    def plot_all_diagnostics(self, save_dir: str = None):
        ...
        control_plot = self.plot_control_values(
            save_path=os.path.join(save_dir, "control_values.png")
        )
