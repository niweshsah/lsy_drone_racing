...
class MyAStarController(Controller):
    ...
    def _calculate_grid_bounds(self) -> None:
        ...
        all_points = np.vstack(
            [self.initial_position, self.gate_positions, self.obstacle_positions]
        )
    ...
    def _create_occupancy_grid(self) -> None:
        ...
        x, y, z = np.indices((2 * brush_radius + 1, 2 * brush_radius + 1, 2 * brush_radius + 1))
        ...
        for obs_idx in obstacle_indices:
            ...
            valid_mask = np.all(
                (cells_to_occupy_indices >= 0) & (cells_to_occupy_indices < self.grid_shape), axis=1
            )
    ...
    def _a_star_search(
        self, start_pos_world: NDArray[np.floating], goal_pos_world: NDArray[np.floating]
        ...
        while open_set:
            ...
            for neighbor_offset in self.A_STAR_NEIGHBORS:
                ...
                if not self._is_valid_grid_index(neighbor_index) or self._is_occupied(
                    neighbor_index
                ):
                    continue
    ...
    def _check_line_of_sight(self, pos1: NDArray[np.floating], pos2: NDArray[np.floating]) -> bool:
    ...
    def _is_valid_grid_index(self, grid_index: tuple[int, int, int]) -> bool:
        ...
        return (
            0 <= grid_index[0] < self.grid_shape[0]
            and 0 <= grid_index[1] < self.grid_shape[1]
            and 0 <= grid_index[2] < self.grid_shape[2]
        )
    ...
    def _generate_trajectory(self, duration: float, waypoints: NDArray[np.floating]) -> CubicSpline:
    ...
    def _visualize_trajectory(
        ...
        if waypoints is not None:
            self.ax.plot(
                ...
                linestyle="--",
                color="blue",
                label=f"A* Waypoints ({len(waypoints)})",
                linewidth=1,
            )
    ...
    def get_trajectory_waypoints(self) -> NDArray[np.floating]:
        ...
        time_samples = np.linspace(
            0, self.TRAJECTORY_DURATION, int(self._control_frequency * self.TRAJECTORY_DURATION)
        )
