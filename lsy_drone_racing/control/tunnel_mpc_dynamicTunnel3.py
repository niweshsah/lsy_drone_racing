...
# (Assumes these modules are available in your python path)
try:
    ...
    # Fallback for standalone testing (mocks)
...
# Use non-interactive backend for saving plots without window popup
matplotlib.use('Agg')
...
CONSTANTS = {
    "v_max_ref": 1.5,           # m/s (Reference speed)
    "corner_acc": 1.95,         # m/s^2 (Centripetal limit)
    "mpc_horizon": 50,          # Steps
    "max_lateral_width": 0.3,   # m (Static Corridor width)
    "safety_radius": 0.1,       # m (Obstacle buffer)
    "tf_horizon": 1.0           # s (Prediction time horizon)
}
...
def symbolic_dynamics_spatial(params: Dict[str, Any]) -> Tuple[ca.MX, ca.MX, ca.MX, ca.MX]:
    """
    Generates the symbolic spatial dynamics for the drone.
    State: [s, w1, w2, ds, dw1, dw2, phi, theta, psi, dphi, dtheta, dpsi]
    Input: [phi_c, theta_c, psi_c, T_c]
    """
    # Unpack parameters
    mass = params["mass"]
    ...
    t_vec = ca.SX.sym('t_vec', 3)   # Tangent
    n1_vec = ca.SX.sym('n1_vec', 3) # Normal 1
    n2_vec = ca.SX.sym('n2_vec', 3) # Normal 2
    k1, k2 = ca.SX.sym('k1'), ca.SX.sym('k2') # Curvature
    dk1, dk2 = ca.SX.sym('dk1'), ca.SX.sym('dk2') # Spatial derivative of curvature
...
class GeometryEngine:
    def __init__(
        ...
        # Generate Parallel Transport Frame
    ...
    def _add_detour_logic(self, wps, types, normals):
        ...
        for i in range(len(wps) - 1):
            ...
            # Detour logic if LEAVING a GATE (Type 1)
            if curr_type == 1: 
                gate_idx = i - 1 
    ...
    def _compute_hermite_tangents(self):
        ...
        for i in range(num_pts):
            ...
            if self.wp_types[i] == 1: 
                ...
                # Auto-Flip normal if flow is reversed
    ...
    def _generate_parallel_transport_frame(self, num_points=3000):
        ...
        # Pass 1: Integrate Frame and Curvature
        k1_list, k2_list = [], []
    ...
    def get_frame(self, s_query: float) -> Dict[str, Any]:
    ...
    def get_closest_s(self, pos_query: np.ndarray, s_guess=0.0, window=5.0) -> float:
        """Finds s* locally around s_guess."""
...
class SpatialMPC:
    def __init__(self, params: Dict[str, Any], N=50, Tf=1.0):
        self.N = N
        self.Tf = Tf
        self.params = params
        self.params['g'] = params['gravity_vec'][2] # Consistency check
    ...
    def _build_solver(self) -> AcadosOcpSolver:
        ...
        # --- COST CONFIGURATION ---
        # Weights: [Pos, Vel, Att, Rate] + Input
        q_diag = np.array([
            1.0, 20.0, 20.0,    # s, w1, w2
            10.0, 5.0,  5.0,    # ds, dw1, dw2
            1.0,  1.0,  1.0,    # rpy
            0.1,  0.1,  0.1     # drpy
        ])
        ...
        # w1, w2 indices in x are 1 and 2; rpy indices 6,7,8
        ...
        # Slack Config
        ns = 2
        ocp.constraints.idxsbx = np.array([0, 1]) # Slack on 0th and 1st element of idxbx (w1, w2)
        ...
        # --- PARAMETERS ---
        # [t_x, t_y, t_z, n1_x, ..., k1, k2, dk1, dk2]
        p0 = np.concatenate([
            [1,0,0], [0,1,0], [0,0,1], 
            [0,0], [0,0]
        ])
...
class SpatialMPCController(Controller):
    def __init__(self, obs: Dict, info: Dict, config: Dict, env=None):
        ...
        # Logging
        self.control_log = {
            k: [] for k in ['timestamps', 'phi_c', 'theta_c', 'psi_c', 
                           'thrust_c', 'solver_status', 's', 'w1', 'w2', 'ds']
        }

        # 5. Pre-calculate Global Visualization Points (Optimization)
        # Subsample points (e.g., every 5th) to keep rendering fast while showing the full track
        subsample = 5
    ...
    def _draw_global_track(self):
        """Draws the entire static track and tunnel boundaries (Green path, Red walls)."""
        if self.env is None: return
        try:
            # 1. Full Ideal Path (Green)
            draw_line(self.env, points=self.global_viz_center, rgba=np.array([0.0, 1.0, 0.0, 0.5]))
            
            # 2. Full Static Tunnel (Red, Transparent)
            # These represent the hard limits of the problem
    ...
    def _compute_corridor_bounds(self, s_pred, frame_pos, frame_t, frame_n1, frame_n2):
        ...
        by projecting obstacles onto the transverse plane.
        """
        # Initialize with full corridor width
        lb_w1, ub_w1 = -self.W1_MAX, self.W1_MAX
        lb_w2, ub_w2 = -self.W2_MAX, self.W2_MAX
        
        # Sensitivity range (longitudinal)
        ...
        for obs_pos in self.obstacles_pos:
            ...
            if abs(s_dist) < longitudinal_threshold:
                ...
                # Check if obstacle is inside our max corridor
                if (lb_w1 < w1_obs < ub_w1) and (lb_w2 < w2_obs < ub_w2):
                    
                    # Pass Logic: Left or Right?
                    if w1_obs > 0:
    ...
    def _get_gate_normals(self, gates_quaternions: np.ndarray) -> np.ndarray:
        ...
        # Assumes gate normal is the local X-axis
        gates_normals = rotation_matrices[:, :, 0] 
        return gates_normals

    def reset_mpc_solver(self):
        """Warm starts the solver with a forward linear guess."""
        nx = 12
    ...
    def compute_control(self, obs: Dict, info: Optional[Dict] = None) -> np.ndarray:
        # --- 1. VISUALIZATION CALL ---
        # Draw the full static track and limits
        ...
        hover_T = self.params['mass'] * -self.params['g'] # Corrected G sign if gravity is negative
        
        # 1. State Feedback (World -> Spatial)
        x_spatial = self._cartesian_to_spatial(obs["pos"], obs["vel"], obs["rpy"], obs["drpy"])
        
        # Update current state (Initial Condition)
        ...
        # Visualization Data Containers for Dynamic Tunnel (The Orange Pinch)
        vis_dynamic_left = []
        vis_dynamic_right = []
        ...
        for k in range(self.mpc.N):
            ...
            # Update Constraints (ignore k=0 as it is the current state)
            if k > 0:
            ...
            # D. Speed Logic (Curvature based slowdown)
        ...
        if self.env is not None and self.debug:
            try:
                ...
                # Draw Dynamic Tunnel (Orange)
                # These are the dynamic constraints the MPC sees for the current horizon
    ...
    def _cartesian_to_spatial(self, pos, vel, rpy, drpy):
        ...
        h = 1 - f['k1'] * w1 - f['k2'] * w2
        h = max(h, 0.01) # Singularity protection
    ...
    def save_control_log(self, filepath: Optional[str] = None):
        if filepath is None:
    ...
    def plot_control_values(self, figsize=(16, 10), save_path: Optional[str] = None):
        if len(self.control_log['timestamps']) == 0:
            return
    ...
    def plot_solver_status(self, save_path: Optional[str] = None):
        if len(self.control_log['timestamps']) == 0:
            return
        t = np.array(self.control_log['timestamps'])
    ...
    def plot_all_diagnostics(self, save_dir: Optional[str] = None):
        if save_dir is None:
            timestamp = self.episode_start_time.strftime("%Y%m%d_%H%M%S")
